...
cauta_strazi_din_oras(OrasCod) ->%%nope codul e pus ok
    Strazi = do(qlc:q([{ X#strada.cod, X#strada.nume,X#strada.oras_id, X#strada.stradatype_id} || X<-mnesia:table(strada), X#strada.oras_id == OrasCod])),
    salveaza_strada(Strazi).

salveaza_toate_strazile() -> %asta a fost de test acum nu mai e chemata
...
salveaza_strada([Ostrada|Restu]) -> 
    ...
    %io:fwrite("Nume e ~s, Codu e ~s, CityId e ~s, Streettype e ~s", [Nume, Cod, CityId, StreetTypeCode]),
    CodStrada = boss_db:find_first(strada_type, [{cod,'matches', StreetTypeCode}]),
    CodOras = boss_db:find_first(city, [{cod,'matches',CityId}]), %cod oras e nimic
    StradaNoua = strada:new(id, unicode:characters_to_binary(Nume), unicode:characters_to_binary(Cod), CodStrada:id(), CodOras:id()),
    ...
    xmerl_sax_parser:file(?XML,[ 
            {event_fun,
             ...
             ({startElement, _, "Street", _Ceva, [{_,_,"code", Cod}, {_,_,"name", Nume}, {_,_, "city_code", CodOras}, {_,_, "streetType", TipStrada}]}, _Location, _State) ->
                     insereaza_strada(Cod, Nume, CodOras, TipStrada); %codu orasului e gresit rau
             (_,_,State) ->
                     State
             end}
              ]),
